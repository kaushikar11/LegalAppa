                ...
                <div className="w-96 text-gray-300 space-y-5 p-4 shadow-xl border rounded-xl border-gray-700 bg-gray-800">
                    ...
                    >
                        ...
                        <button
                            ...
                            className={`w-full px-4 py-2 text-white font-medium rounded-lg ${isSigningIn ? 'bg-gray-600 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 hover:shadow-xl transition duration-300'}`}
                        >
                            {isSigningIn ? 'Signing In...' : 'Sign In'}
                        </button>
                    </form>
                    <p className="text-center text-sm text-gray-400">Don't have an account? <Link to={'/register'} className="hover:underline font-bold text-indigo-400">Sign up</Link></p>
                    <div className='flex flex-row text-center w-full'>
                        <div className='border-b-2 mb-2.5 mr-2 w-full border-gray-700'></div><div className='text-sm font-bold text-gray-400 w-fit'>OR</div><div className='border-b-2 mb-2.5 ml-2 w-full border-gray-700'></div>
                    </div>
                    <button
                        disabled={isSigningIn}
                        onClick={(e) => { onGoogleSignIn(e) }}
                        className={`w-full flex items-center justify-center gap-x-3 py-2.5 border rounded-lg text-sm font-medium text-gray-200 ${isSigningIn ? 'cursor-not-allowed' : 'hover:bg-gray-700 transition duration-300 active:bg-gray-700'}`}>
